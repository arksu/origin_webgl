package com.origin.generator

import com.origin.GRID_SIZE
import com.origin.SUPERGRID_SIZE
import com.origin.generator.voronoi.Point
import com.origin.generator.voronoi.Voronoi
...
object MapGenerator {
    ...
    fun run() {
        ...
        println("make tiles...")
        layer(image, 250.0, 0.74, TileColors.SWAMP)
        layer(image, 300.0, 0.8, TileColors.SWAMP)
        layer(image, 300.0, 0.76, TileColors.CLAY)
        layer(image, 400.0, 0.8, TileColors.CLAY)
        layer(image, 170.0, 0.6, TileColors.MEADOW_LOW)
        layer(image, 220.0, 0.72, TileColors.PRAIRIE)
        layer(image, 170.0, 0.8, TileColors.TUNDRA)
        layer(image, 200.0, 0.1, TileColors.FOREST_PINE)
        layer(image, 260.0, 0.2, TileColors.FOREST_PINE)
        fill(image, TileColors.FOREST_LEAF)
        ...
        for (e in diagram.edges.subList(5, diagram.edges.size - 1)) {
            ...
            if (dropEdges[idx])
                drawLine(image, pointsToDraw[idx], sizePointsToSand[idx], TileColors.SAND)
            idx++
        }
        ...
        for (e in diagram.edges.subList(5, diagram.edges.size - 1)) {
            if (dropEdges[idx])
                drawLine(image, pointsToDraw[idx], sizePointsToWater[idx], TileColors.WATER)
            idx++
        }
        ...
        idx = 0
        for (l in lakes) {
            drawCircle(image, l.x.toInt(), l.y.toInt(), lakesSize[idx].x.toInt(), lakesSize[idx].y.toInt(), TileColors.WATER)
            idx++
        }
        ...
        for (e in diagram.edges.subList(5, diagram.edges.size - 1)) {
            if (dropEdges[idx])
                drawLine(image, pointsToDraw[idx], sizePointsToDeepWater[idx], TileColors.WATER_DEEP)
            idx++
        }
    ...
    fun makeLake(img: BufferedImage, x: Int, y: Int, radius0: Int, radius1: Int) {
        val g = img.graphics
        g.color = Color(TileColors.WATER)
